...
class ModelMyocCopu extends Model {
	...
	public function getUploads($data) {
		if(isset($data['customer_id'])) {
			$query = $this->db->query("SELECT u.* FROM " . DB_PREFIX . "myoc_customer_upload cu LEFT JOIN `" . DB_PREFIX . "upload` u ON (cu.upload_id = u.upload_id) WHERE cu.customer_id = '" . (int)$data['customer_id'] . "' ORDER BY u.date_added DESC");
		}
		if(isset($data['order_id'])) {
			$query = $this->db->query("SELECT u.* FROM " . DB_PREFIX . "myoc_order_upload ou LEFT JOIN `" . DB_PREFIX . "upload` u ON (ou.upload_id = u.upload_id) WHERE ou.order_id = '" . (int)$data['order_id'] . "' ORDER BY u.date_added DESC");
		}
		return is_object($query) ? $query->rows : array();
	}
	...
	public function upgradeTable()
	{
		...
		//from v1.5.x
		$query = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "myoc_upload'");
		if($query->num_rows) {
			$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "myoc_upload`");
		}
	}
}
?>