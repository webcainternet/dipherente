...
class ModelMyocCopu extends Model {
	public function getTotalUploads($data) {
		if(isset($data['type']) && isset($data['type_id'])) {
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "myoc_" . $data['type'] . "_upload WHERE " . $data['type'] . "_id = '" . (int)$data['type_id'] . "'");
	...
	public function getUploads($data) {
		if(isset($data['customer_id'])) {
			...
			$query = $this->db->query("SELECT u.*,cu.* FROM " . DB_PREFIX . "myoc_customer_upload cu LEFT JOIN `" . DB_PREFIX . "upload` u ON (cu.upload_id = u.upload_id) WHERE cu.customer_id = '" . (int)$data['customer_id'] . "' ORDER BY u.date_added DESC LIMIT " . (int)$data['start'] . "," . (int)$data['limit']);
		}
		if(isset($data['order_id'])) {			
			$query = $this->db->query("SELECT u.*,ou.* FROM " . DB_PREFIX . "myoc_order_upload ou LEFT JOIN `" . DB_PREFIX . "upload` u ON (ou.upload_id = u.upload_id) WHERE ou.order_id = '" . (int)$data['order_id'] . "' ORDER BY u.date_added DESC");
		}
		$uploads = array();
	...
	public function getUpload($upload_id) {
		$query = $this->db->query("SELECT u.*,cu.* FROM " . DB_PREFIX . "upload u LEFT JOIN `" . DB_PREFIX . "myoc_customer_upload` cu ON (cu.upload_id = u.upload_id) WHERE u.upload_id = '" . (int)$upload_id . "'");

		return $query->row;
	}

	public function editUpload($upload_id, $data = array()) {
		return $this->db->query("UPDATE " . DB_PREFIX . "upload SET `filename` = '" . $this->db->escape($data['filename']) . "' WHERE upload_id = '" . (int)$upload_id . "'");
	}
	...
	public function getOrderUploadInvoice($data) {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/myoc/copu_mail.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/myoc/copu_mail.tpl', $data);
		} else {
			return $this->load->view('default/template/myoc/copu_mail.tpl', $data);
		}
	}
