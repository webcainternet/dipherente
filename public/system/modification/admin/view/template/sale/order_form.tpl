...
<?php if($copu_order_html) { ?><li class="disabled"><a href="#tab-copu" data-toggle="tab"><?php echo $tab_copu; ?></a></li><?php } ?>
          </ul>
	...
	'select': function(item) {
		...
		if (item['option'] != '') {
			...
			for (i = 0; i < item['option'].length; i++) {
				...
				if (option['type'] == 'file') {
if(option['option_id'] == <?php echo $copu_product_option_ids; ?>) { <?php echo $copu_product_html; ?> } else {
					html += '<div class="form-group' + (option['required'] ? ' required' : '') + '">';
					html += '  <label class="col-sm-2 control-label">' + option['name'] + '</label>';
					html += '  <div class="col-sm-10">';
...
$('#button-save').on('click', function() {
	...
	$.ajax({
		url: url,
		type: 'post',
		data: $('#tab-total select[name=\'order_status_id\'], #tab-copu input[name*=\'upload[]\'], #tab-total select, #tab-total textarea[name=\'comment\'], #tab-total input[name=\'affiliate_id\']'),
		dataType: 'json',
		beforeSend: function() {
