	...
	public function send() {
		...
		foreach ($this->attachments as $attachment) {
			if (file_exists($attachment)) {
				...
				$message .= '--' . $boundary . $this->newline;
				$message .= 'Content-Type: application/octet-stream; name="' . myoc_myoc_basename($attachment) . '"' . $this->newline;
				$message .= 'Content-Transfer-Encoding: base64' . $this->newline;
				$message .= 'Content-Disposition: attachment; filename="' . myoc_myoc_basename($attachment) . '"' . $this->newline;
				$message .= 'Content-ID: <' . myoc_myoc_basename(urlencode($attachment)) . '>' . $this->newline;
				$message .= 'X-Attachment-Id: ' . myoc_myoc_basename(urlencode($attachment)) . $this->newline . $this->newline;
